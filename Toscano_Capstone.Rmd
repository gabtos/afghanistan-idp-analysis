...
## Q1: Data Overview 
### (1.1) Load the UNHCR_Survey data.
...
idp_populations <- read.csv("worldbank_data/world-bank-idp-populations.csv", skip = 4)
idp_populations <- idp_populations[idp_populations$Country.Code == "AFG", ] %>% 
  select(starts_with("X"))

#Cleanup and pivot data
start_year = 1960
end_year = 2022
year_list <- paste(seq(start_year, end_year), sep = "")
...
idp_populations <- pivot_longer(idp_populations, cols = everything(), names_to = "year", values_to = "num_idp") %>% 
  drop_na()

ggplot(idp_populations, aes(x = year, y = num_idp, group=1)) +
  geom_line() +
...
The UNHCR_Survey data is already filtered to only IDP records
...
top_10_districts <- un_survey %>% 
  dplyr::select(district, total_members) %>% 
  group_by(district) %>% 
  summarize(num_idp = sum(total_members)) %>% 
  arrange(-num_idp) %>% 
  slice(1:10)
...
```{r Top Provinces}
  ...
  ggplot(top_10_districts) +
  geom_col(aes(x = reorder(district, num_idp), y = num_idp)) +
    labs(
